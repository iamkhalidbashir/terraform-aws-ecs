...
resource "aws_security_group" "instance" {
  ...
  vpc_id      = "${var.vpc_id}"
  tags        = "${merge(var.tags, { Name = "${var.name}-container-instance"})}"
}
...
resource "aws_launch_configuration" "instance" {
  ...
  key_name             = "${var.instance_keypair != "" ? var.instance_keypair : element(concat(aws_key_pair.user.*.key_name, tolist("")), 0)}"
